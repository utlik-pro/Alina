...
def _ig_channel_brief(known_phone: str = "") -> str:
    ...
    return (
        ...
        "- ⏰ NIGHT SHIFT BOOKS FOR THE NEXT WORKING DAY. You are answering "
        "in the middle of the night; the salon works 10:00 AM–9:00 PM. Book "
        "for the NEXT time the salon is open, never 'right now' and never "
        "the evening that has already passed. If the client says 'today' "
        "after 9 PM, gently point them to the coming day: 'Today we are "
        "already closed dear 🌹 for tomorrow we have …'. The slot list you "
        "are given is already filtered by the real clock — offer only what "
        "is in it, and never promise a visit within the next couple of "
        "hours. IMPORTANT after midnight: the working day that is about to "
        "start IS 'today' — if the list still has today's times, offer them "
        "normally (a client writing at 2 AM wants that same day, don't "
        "push them a day further).\n"
